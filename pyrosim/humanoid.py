...
class ROBOT:
    ...
    def Send_To_Simulator(self, sim, color, biasValues):
        ...
        self.eyes = EYES(self.head_ID, [0, -1.5*c.R, 4*c.L+2.3*c.R], 0.02, [1,0,0], [0,-1,0], 0.015)
    ...
    def Send_Objects(self, sim, color):
        ...
        # feet
        sim.Send_Box(objectID = self.num_objects , x=-.5*c.L, y=0, z=.5*c.R, length=.6*c.L, width=.6*c.L,
         height=c.R, r=color[0], g=color[1], b=color[2], mass=0.35)
        ...
        sim.Send_Cylinder(objectID = self.num_objects , x=-.5*c.L, y=0, z=c.L+c.R, r1=0, r2=0, r3=1, length=2*c.L, radius=c.R, r=color[0], g=color[1], b=color[2])
        ...
        #torso
        sim.Send_Box(objectID = self.num_objects , x=0, y=0, z=3*c.L, length=1.5*c.L, width=.5*c.L,
         height=2*c.L, r=color[0], g=color[1], b=color[2])

        self.num_objects += 1
        
        #shoulders
        sim.Send_Sphere(objectID = self.num_objects, x= -.75*(c.L+c.R), y= 0, z= 4*c.L-c.R, 
            mass=0.1, radius = c.R,  r=color[0], g=color[1], b=color[2])
        ...
        sim.Send_Cylinder(objectID = self.num_objects , x=-.75*(c.L+c.R), y=0, z=3*c.L-c.R, r1=0, r2=0, r3=1, length=2*c.L, radius=.5*c.R, r=color[0], g=color[1], b=color[2], mass=0.5)
        ...
        #head
        sim.Send_Sphere(objectID = self.num_objects, x= 0, y= 0, z= 4*c.L+2*c.R, 
	...
	#feet to legs
        ...
        #shoulder to arms        
        sim.Send_Joint(jointID = self.num_joints, firstObjectID = 5, secondObjectID = 7,
        ...
        #torso to head
        sim.Send_Joint(jointID = self.num_joints, firstObjectID = 4, secondObjectID = 9,
         n1 =0, n2 =1 , n3 =0, x=0, y=0, z=4*c.L, lo=-0.1, hi=0.1)
        self.num_joints += 1
    ...
    def Send_Sensors(self, sim):
        ...
        sim.Send_Position_Sensor(sensorID = self.num_sensors-1, objectID = 9)
