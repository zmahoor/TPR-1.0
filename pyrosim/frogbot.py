...
class ROBOT:
    ...
    def Send_Objects(self, sim, color):
        ...
        sim.Send_Cylinder(objectID = self.num_objects, x=-0.5*(c.L+c.R), y=(c.L/2-c.R)-math.sin(math.pi/4)*(.5*c.L+c.R), z=(c.L+2*c.R+math.sin(math.pi/4)*(.5*c.L+c.R))/2, r1=0 , r2=0, r3=1,
    ...
    def Send_Joints(self, sim):
        ...
        sim.Send_Joint(jointID = self.num_joints, firstObjectID = 0, secondObjectID = 1,
         n1 =1, n2 =0 , n3 =0, x=-0.5*(c.L+c.R), y=(c.L/2-c.R)+math.sin(math.pi/4)*(.5*c.L+c.R), z=(c.L+2*c.R)-math.sin(math.pi/4)*(.5*c.L+c.R))
        self.num_joints += 1
